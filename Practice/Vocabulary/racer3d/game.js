...
(() => {
  ...
  function createLanePortal(
    ...
    baseLeft.position.set(
      -1.3,
      .08,
      0
    );

    baseRight.position.set(
      1.3,
      .08,
      0
    );
  ...
  function startRound(){
    ...
    gateWave.userData.item=
      item;

    gateWave.userData.resolved=
      false;
    ...
    .forEach(
      (
        ...
        portal.userData.correct=
          answer===item.correct;

        gateWave.userData.portals.push(
          portal
        );
  ...
  function updateGate(
    ...
    if(
      !gateWave.userData.resolved
      &&
      gateWave.position.z>=RESOLVE_Z
    ){
      gateWave.userData.resolved=
        true;

      const chosen=
        gateWave.userData.portals.find(
          portal=>
          portal.userData.lane
          ===
          targetLane
        );

      if(
        chosen?.userData.correct
      ){
        correctAnswer();
      }else{
        wrongAnswer();
  ...
  function wrongAnswer(){
    ...
    flashMessage(
      both
      ?
      "✕ Sai! Cả 2 xe đối thủ vượt lên!"
      :
      "✕ Sai! Một xe đối thủ vượt lên!",
      "bad"
    );
